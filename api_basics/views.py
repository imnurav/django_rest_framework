...
from .serializers import ArtcileSerializer
...
class ArticleViewSet(viewsets.ViewSet):
    def list(self, request):
        ...
        serializer = ArtcileSerializer(articles, many=True)
        return Response(serializer.data)

    def create(self, request):
        serializer = ArtcileSerializer(data=request.data)
    ...
    def retrieve(self, request, pk=None):
        ...
        serializer = ArtcileSerializer(article)
        return Response(serializer.data)

    def update(self, request, pk=None):
        article = Article.objects.get(pk=pk)
        serializer = ArtcileSerializer(article, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
